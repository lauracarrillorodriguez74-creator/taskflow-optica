// TASKFLOW · App — Auth-only
// currentUser viene SOLO de Supabase auth → workers.auth_id
// Sin UserSwitcher, sin currentUserId, sin workers[0]
...
function checkIsManager(worker) {
  ...
  return MANAGER_NAMES.some(m => (worker.name || "").toLowerCase().startsWith(m));
}
...
const TWEAK_DEFAULTS = /*EDITMODE-BEGIN*/{
  "accent": "blue", "density": "regular", "defaultView": "kanban",
  "theme": "light", "cards": "default", "frame": "ipad"
}/*EDITMODE-END*/;
...
/* ════════════════════════════════════════════════
   SIDEBAR
════════════════════════════════════════════════ */
function Sidebar({ active, setActive, currentUser, workers, tasks }) {
  const counts = {
  ...
  const navItems = NAV_ITEMS.filter(n => !n.mgr || isMgr);
    ...
    <nav className="tf-side">
      ...
      {navItems.map(item => (
        <div key={item.id}
          ...
          {counts[item.id] > 0 && <span className="tf-count">{counts[item.id]}</span>}
        </div>
      ))}
      ...
      <div style={{ marginTop: "auto", paddingTop: 12, borderTop: "1px solid var(--tf-line)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: 8, borderRadius: 10, background: "var(--tf-bg-2)" }}>
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: "var(--tf-mute)", display: "flex", alignItems: "center", gap: 4 }}>
              {currentUser.role}
              {isMgr && <span className="tf-mgr-badge">Gerente</span>}
            </div>
          </div>
        </div>
...
/* ════════════════════════════════════════════════
   TOP BAR
════════════════════════════════════════════════ */
function TopBar({ active, onCreate, currentUser }) {
  const titles = {
  ...
  return (
    <header className="tf-top">
      ...
      <button
        onClick={doSignOut}
        style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 12px", borderRadius: 9, border: "1px solid var(--tf-line)", background: "transparent", color: "var(--tf-red-500)", fontWeight: 600, fontSize: 12.5, fontFamily: "inherit", cursor: "pointer" }}
      >
...
/* ════════════════════════════════════════════════
   PANTALLA: usuario no vinculado
════════════════════════════════════════════════ */
function UnlinkedUserScreen() {
  const email = window.__currentEmail || "tu cuenta";
  return (
    <div style={{ position: "fixed", inset: 0, display: "grid", placeItems: "center", background: "linear-gradient(160deg,#F4F7FA 0%,#E6EEF7 100%)", fontFamily: '"Inter",system-ui,sans-serif', padding: 24 }}>
      <div style={{ width: "100%", maxWidth: 440, textAlign: "center", background: "rgba(255,255,255,.92)", backdropFilter: "blur(20px)", border: "1px solid rgba(255,255,255,.7)", borderRadius: 22, padding: "40px 32px", boxShadow: "0 30px 80px rgba(11,47,91,.14)" }}>
        <div style={{ width: 56, height: 56, borderRadius: 16, background: "#FEF2F2", border: "1px solid #FAD2D6", display: "grid", placeItems: "center", margin: "0 auto 20px" }}>
        ...
        <div style={{ fontFamily: '"Inter Tight",sans-serif', fontWeight: 700, fontSize: 20, marginBottom: 10, color: "#1F2933" }}>Usuario no vinculado</div>
        <p style={{ fontSize: 14, color: "#3E4C5E", lineHeight: 1.6, margin: "0 0 8px" }}>
          Tu cuenta <strong style={{ color: "#0B2F5B" }}>{email}</strong> no está asociada a ningún trabajador.
        </p>
        <p style={{ fontSize: 13, color: "#6B7888", lineHeight: 1.5, margin: "0 0 24px" }}>
          Pide al gerente que rellene <code style={{ background: "#F4F7FA", padding: "1px 6px", borderRadius: 4, fontSize: 12 }}>workers.auth_id</code> con tu UUID de Supabase Auth.
        </p>
        <button onClick={doSignOut} style={{ padding: "11px 22px", borderRadius: 11, border: "none", background: "linear-gradient(160deg,#1E5B94,#0B2F5B)", color: "#fff", fontWeight: 600, fontSize: 14, fontFamily: "inherit", cursor: "pointer" }}>
          Cerrar sesión
        </button>
      </div>
    </div>
...
function App() {
  ...
  // currentUser SOLO desde auth_id. Sin fallback a workers[0] ni localStorage.
  ...
  if (!currentUser) return <UnlinkedUserScreen />;

  return (
    <div className="tf-app">
      <Sidebar active={active} setActive={setActive} currentUser={currentUser} workers={workers} tasks={visibleTasks}/>
      <div className="tf-shell-inner">
        <TopBar active={active} onCreate={handleCreate} currentUser={currentUser}/>
          ...
          {active === "management" && !checkIsManager(currentUser) && (
            <div style={{ display: "grid", placeItems: "center", height: "100%", minHeight: 400 }}>
              <div style={{ textAlign: "center", padding: 40 }}>
                ...
                <div style={{ color: "var(--tf-mute)", fontSize: 13.5, maxWidth: 320, lineHeight: 1.7 }}>Este panel está disponible solo para gerentes.</div>
              </div>
            </div>
          )}
        </main>
      ...
      <TaskDrawer
        ...
        workers={workers} history={history} currentUser={currentUser}
        onClose={handleClose} onSave={handleSave}
        onDelete={handleDelete} onStatusChange={updateTaskStatus}
      />
      ...
      <TweaksPanel>
        ...
        <TweakRadio label="Vista de tareas" value={t.defaultView} options={[{value:"kanban",label:"Kanban"},{value:"list",label:"Lista"}]}                                onChange={v => setTweak("defaultView",v)}/>
        <TweakRadio label="Marco" value={t.frame} options={[{value:"ipad",label:"iPad"},{value:"fullscreen",label:"Pantalla"}]}
...
/* ── Root: AuthGate envuelve TODO ────────────────── */
function Root() {
  if (!window.__SUPABASE_READY || !window.AuthGate) {
    return (
      <div style={{ padding: 30, fontFamily: "Arial", color: "red" }}>
        Error: Supabase no configurado o auth.jsx no cargado. Revisa index.html.
      </div>
    );
  }
  return (
